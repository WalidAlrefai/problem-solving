...
const objLat = (obj) => {
    var firstname = obj.firstName.charAt(0).toUpperCase() + obj.firstName.slice(1);
...
const applicationsStatics = (arr) => {
    let result = {
        ...
        // var counterReject ;
        for(let i = 0 ; i <arr.length ; i++){
            if(((arr[i].firstName ==null &&arr[i].firstName == " ") && (arr[i].lastName == null && arr[i].lastName == " ")) || arr[i].yearsOfExperience > 1){
                
                result.rejectApplicants ++ ;
...
const classesAvg = (data) => {
    for(let i=0 ; i < data.grades.length ; i++ ){
        for(let j=0 ; j < data.grades[i].classes.length ; j++){
            ...
            data.grades[i].classes[j].classScores.avg= sum/ data.grades[i].classes[j].classScores.length
            return  Math.floor(data.grades[i].classes[j].classScores.avg)
